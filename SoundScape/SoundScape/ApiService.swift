...
class APIService {
    ...
    private static let backendURL = "http://localhost:8085/music-diary" // 백엔드 API URL
    ...
    static func fetchAlbumDetailsById(for trackId: String, completion: @escaping (AlbumDetails?) -> Void) {
        let url = URL(string: "http://localhost:8085/spotify/track/\(trackId)")! // 음악 아이디로 요청 URL 변경
        var request = URLRequest(url: url)
        request.httpMethod = "GET"
    ...
    func fetchDiaryEntries(for userId: String, completion: @escaping (Result<[MusicDiaryEntry], Error>) -> Void) {
        guard let url = URL(string: "http://localhost:8085/api/entries?loginId=\(userId)") else {
            DispatchQueue.main.async {
                completion(.failure(NSError(domain: "Invalid URL", code: -1, userInfo: nil)))
            }
            return
